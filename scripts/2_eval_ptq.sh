...
RUN_ANALYSIS_FLAG=0 # Track if --run_analysis was passed
LAYER_LIST_PATH="layers_to_rotate.json" # Default path for the JSON list
R3_ONLY_SHELL_FLAG=0
OPTIMIZED_ROTATION_PATH_VALUE=""
NB_EVAL_RUNS=1
EXCLUDE_ACTIVATION_LAYER_LIST=""
EVAL_DATASET="wikitext2" # Default evaluation dataset
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    --online_r3_only) 
      ...
      shift 
      ;;
    --exclude_activations_layers)
      shift # consume the flag
      while [[ $# -gt 0 ]] && [[ "$1" != --* ]]; do
        EXCLUDE_ACTIVATION_LAYER_LIST+="$1 "
        shift
      done
      ;;
    --layer_list_file)
    ...
    --run_analysis)
      ...
      ;;
    --eval_dataset)
      if [[ -n "$2" ]] && [[ "$2" != --* ]]; then EVAL_DATASET="$2"; shift 2;
      else echo "ERROR: --eval_dataset requires a dataset name." >&2; exit 1; fi ;;
    --timing_output_path) 
...
PYTHON_ARGS+=" --access_token \"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\"" # Quote token
...
if [[ -n "$EXCLUDE_ACTIVATION_LAYER_LIST" ]]; then
  PYTHON_ARGS+=" --exclude_activations_layers $EXCLUDE_ACTIVATION_LAYER_LIST"
  echo "INFO: EXCLUDING activations from quantization for layers: $EXCLUDE_ACTIVATION_LAYER_LIST"
fi

# Conditionally add the run_analysis flag for ptq.py
if [[ $RUN_ANALYSIS_FLAG -eq 1 ]]; then
    PYTHON_ARGS+=" --run_analysis"
    echo "INFO: Shell: --run_analysis detected. Enabling analysis for ptq.py."
fi

# Conditionally add the evaluation dataset
if [[ -n "$EVAL_DATASET" ]]; then
    PYTHON_ARGS+=" --eval_dataset \"$EVAL_DATASET\""
    echo "INFO: Shell: Using evaluation dataset: $EVAL_DATASET"
else
    echo "INFO: Shell: No evaluation dataset specified. Defaulting to 'wikitext2'."
fi
...
eval torchrun --nnodes=1 --nproc_per_node=1 --rdzv_endpoint="localhost:29501" ptq.py $PYTHON_ARGS